...
function [EMG, EMG_label, imu_data, imu_label, emg_data_set, imu_data_set, data_set] = buildDataSet(root, dir, time, order, DS_len)
...
%     [b_c, a_c] = iircomb(N, BW / (fs / 2), Apass);
%     for i = 1 : 4
...
%     legend('CH0', 'CH1', 'CH2', 'CH3', 'Label');
    ...
    %% IMU 数据
    fs_imu = 120;
    imu_path = ".\" + root + "\" + dir + "\Channel_0\IMU " + time + " Exp.txt";
    imu_label_path = ".\" + root + "\" + dir + "\Channel_0\IMU_Label " + time + " Exp.txt";
    ...
    for c = 1 : classes
        ...
        for i = 1 : 3
            ...
            featureStack = [];
            for j = 0 : DS_len
                rms = featureRMS(iimu(strideWindow * w_s * j + 1: strideWindow * w_s * j + timeWindow * w_s, :));
                mav = featureMAV(iimu(strideWindow * w_s * j + 1: strideWindow * w_s * j + timeWindow * w_s, :));
                ssc = featureSSC(iimu(strideWindow * w_s * j + 1: strideWindow * w_s * j + timeWindow * w_s, :));
                wl = featureWL(iimu(strideWindow * w_s * j + 1: strideWindow * w_s * j + timeWindow * w_s, :));
                featureStack = [rms, mav, wl, ssc];
                feature_i(:, end + 1) = featureStack';
            end
            feature_imu{i} = feature_i;
        end
        emg_data_set{c} = [feature_c{1}; feature_c{2}; feature_c{3}; feature_c{4}];
        imu_data_set{c} = [feature_imu{1}; feature_imu{2}; feature_imu{3}];
        data_set{c} = [feature_c{1}; feature_c{2}; feature_c{3}; feature_c{4}; feature_imu{1}; feature_imu{2}; feature_imu{3}];
    end
end